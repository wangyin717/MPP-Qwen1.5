...
class Minigpt4Qwen(Blip2Base):
    ...
    def __init__(
        ...
        # Qwen中eod_id 对应 Qwen2中的 pad_token_id
    ...
    def preprocess(
            ...
            # print("#####################################################", tokenizer)
            # Qwen2的im_start_id 和 im_end_id 是 additional_special_tokens_ids 中的 [151644, 151645]
            im_start = tokenizer.additional_special_tokens_ids[0]
            im_end = tokenizer.additional_special_tokens_ids[1]
            # im_start = tokenizer.im_start_id
            # im_end = tokenizer.im_end_id
            # testtt1 = tokenizer('\n')
            # testtt2 = tokenizer('system')
            # testtt3 = tokenizer('user')
            # testtt4 = tokenizer('assistant')
            ...
            for i, source in enumerate(sources):
                ...
                for j, sentence in enumerate(source):
                    ...
                    if "<ImageHere>" in content and role == '<|im_start|>user':
                        ...
                        # ttst1 = tokenizer(content)
                        # ttst2 = tokenizer(content).input_ids
                        _input_id = tokenizer(role).input_ids + nl_tokens + \
    ...
    def forward(self, samples):
        ...
        modified_embeds[replace_image_idxs[0], replace_image_idxs[1]] = inputs_llm.view(-1, channels).to(inputs_embeds.dtype)
